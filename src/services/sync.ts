...
export const sincronizarDatos = async (userId?: string) => {
  ...
  try {
    ...
    if (userId) {
      ...
      if (progress) {
        // Ojo: Aquí idealmente haríamos un "Merge" inteligente.
        // Por simplicidad, asumimos que Supabase tiene la verdad absoluta al iniciar.
        await db.runAsync("DELETE FROM user_progress WHERE user_id = ?", [
          userId,
        ]);
