...
export default function EscribirSenaScreen() {
  ...
  const verificarRespuesta = () => {
    ...
    const inputNormalizado = textoUsuario
      .trim()
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
    const correctaNormalizada = palabraCorrecta
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
  ...
  if (!palabras.length)
    return <Text className="text-center mt-10">No hay datos para jugar.</Text>;
  ...
  return (
    <SafeAreaView className="flex-1 bg-secondary-200">
      <Stack.Screen
        options={{ title: "Escribir la Seña", headerBackTitle: "Juegos" }}
      />
      ...
      >
        ...
        {/* SMARTMEDIA: Maneja video/imagen offline */}
        <View className="h-64 bg-white rounded-3xl overflow-hidden mb-8 border border-gray-200 items-center justify-center shadow-sm">
          <SmartMedia
            uri={actual.media_url}
            type={actual.media_type}
            resizeMode="contain"
            autoPlay={true}
            isLooping={true}
        ...
        <TouchableOpacity
          ...
          className="mt-4"
        >
          <Text className="text-center text-gray-400 font-work-regular">
            No sé la respuesta
          </Text>
        </TouchableOpacity>
      </KeyboardAvoidingView>
